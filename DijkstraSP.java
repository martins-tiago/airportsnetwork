...
public class DijkstraSP {
    ...
    public DijkstraSP(EdgeWeightedDigraph G, int s, int custoS, Aviao aviao) {
        for (DirectedEdge e : G.edges()) {
            if (e.weight() < 0) {
                throw new IllegalArgumentException("edge " + e + " has negative weight");
            }
    ...
    private void relax(DirectedEdge e, int custoS, Aviao aviao) {
        int v = e.from(), w = e.to();
        double custo = 0;
        switch (custoS) {
            case 1:

                custo = aviao.calculatempo(e);

                break;
            case 2:
                custo = aviao.calculaeuros(e);
